...
/* Modified by Ben Song. Old recursive scheme is abondoned because it 
   consumes too much stack space */
	    ...
	    )
{
  ...
  while (top != 0) {
    while (first_kid[current] != -1) {
      current = first_kid[current];
      stack[top ++] = current;
    } 
    ...
    do {
      ...
      /*      printf("post[%d] = %d\n", current, postnum - 1); */
    } while (next_kid[current] == -1 && top != 0);
...
)
{
	...
	/*
	stackValue = (int*) calloc (n+1, sizeof(int));
	stackVisit1st = (int*) calloc (n+1, sizeof(int));
	stackVisit2nd = (int*) calloc (n+1, sizeof(int));
	*/
	stack = mxCallocInt (n+1);
