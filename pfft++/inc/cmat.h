...
class Matrix 
{
  ...
  public:
    ...
    bool empty(void) const { return mn_==0; }
    ...
    // resize might cause some memory leak. This should be used by some matrix only
    void resize(Subscript M, Subscript N)
      {
	m_ = M;
	n_ = N;
      }
...
template <class T>
inline Vector<T> operator*(const Vector<T> &x, const Matrix<T>  &A)
{
    return matmult(x, A);
}

// * In-place inverts a matrix using guass-jordan.
// * This function does not have pivoting. 
template <class T>
void invert(Matrix<T>& mat)
{
  assert(mat.num_rows() == mat.num_cols());

  for(size_t i=0; i < mat.num_rows(); i++) {
    /* First i^{th} column of A. */
    assert(abs(mat[i][i]) > DBL_EPSILON);

    T normal = 1./ mat[i][i];
    for(size_t j=0; j < mat.num_rows(); j++) {
      mat[j][i] = mat[j][i] * normal;
    }
    mat[i][i] = normal;
    ...
    for(size_t j=0; j < mat.num_cols(); j++) {
      if(j != i) {
	T multiplier = -mat[i][j];
	for(size_t k=0; k < mat.num_rows(); k++) {
	  T tmp = mat[k][i] * multiplier;
	  if(k != i) {
	    mat[k][j] = mat[k][j] + tmp;
	  } else {
	    mat[k][j] = tmp;
