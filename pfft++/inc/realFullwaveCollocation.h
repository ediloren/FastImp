 ...
 ******************************************************************************
  ...
  This is a specialized version of fullwaveCollocation.h. 
  ...
  const static char cvsid[] = "$Id: realFullwaveCollocation.h,v 1.2 2003/01/10 19:41:04 zhzhu Exp $";

  ==========================================================================
*/

#ifndef _REAL_FULLWAVE_COLLOCATION_H_
#define _REAL_FULLWAVE_COLLOCATION_H_
...
#include "calcpForEkrOverR.h"

namespace pfft {

  template <class KT, class Panel>

  class realFullwaveCollocation : public KernelIntegration {

  public:
    
    realFullwaveCollocation (void) {};
    realFullwaveCollocation (const std::vector<DifferentialOperator>&,
  ...
  private:
    ...
    CalcpForEkrOverR<KT, Panel> calcp;
    KT K_;
    ...
    double compHyperSingularIntegral (const Panel& srcPanel, 
  ...
  /**********************************************************************
   * realFullwaveCollocation --
   **********************************************************************/
  template <class KT, class Panel>
  realFullwaveCollocation<KT, Panel>::
  realFullwaveCollocation (
  ...
  { 
    ...
    if (numGaussQuadraturePoint == 0) {
      ...
      calcp = CalcpForEkrOverR<KT, Panel>(K_, evalPointDirection, slpWanted, 
  ...
  template <class KT, class Panel>
  double
  realFullwaveCollocation<KT, Panel>::result (
					  const size_t integralIndex) const
  {
    ...
    case D_DN_DOUBLE_LAYER:
      ...
      break;
    default:
      throw std::domain_error("realFullwaveCollocation.h: unknown resultType");
      break;
    }
  }
  ...
  template <class KT, class Panel>
  void
  realFullwaveCollocation<KT, Panel>::setupResultTypeList (
						       void)
  {
   ...
   **********************************************************************/
  template <class KT, class Panel>
  typename realFullwaveCollocation<KT, Panel>::ResultType   
  realFullwaveCollocation<KT, Panel>::
  mapDiffOperatorToResultType (
			       const IntegralType integralType) const
  {
	...
	break;
      ...
      case D_DY:
      case D_DZ:
	throw std::domain_error("realFullwaveCollocation.h: can not handle this integral type");
      default:
	throw std::domain_error("realFullwaveCollocation.h: unknown inner deferential operator");
      }
      break;
    case D_DN:
      switch(integralType.second) {
	...
	throw std::domain_error("realFullwaveCollocation.h: unknown inner deferential operator");
      ...
      break;
    default:
      throw std::domain_error("realFullwaveCollocation.h: unknown OUTER deferential operator");
      break;
    }
    return resultType;
  }
  ...
  template <class KT, class Panel>
  void 
  realFullwaveCollocation<KT, Panel>::operator () (
			     ...
			     const Panel& evalPanel) const
  {
    ...
    CalcpForEkrOverR<KT, Panel> calcp(K_, evalPointDirection, slpWanted, 
				       dlpWanted, gradWanted);

    double fs, dfdn1, dfdn2;
    vector3D<double> grad_f;
  ...
  template <class KT, class Panel>
  bool
  realFullwaveCollocation<KT, Panel>::IsWanted (
					    ResultType rt) 
  {
  ...
  template <class KT, class Panel>
  bool
  realFullwaveCollocation<KT, Panel>::IsWantedFromCalcp (
						     ResultType rt) 
  {
