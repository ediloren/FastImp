 ...
 ******************************************************************************
  ...
  Author: Zhenhai Zhu,  Ben Song and Michael Altman
  
  Description:

  This is a specialized version of calcpForEikrOverR. 
  The corresponding kernel is defined in ekrOverR.h
  ...
  const static char cvsid[] = "$Id: calcpForEkrOverR.h,v 1.2 2003/01/10 19:40:51 zhzhu Exp $";

  ==========================================================================
*/

#ifndef _CALCP_FOR_EKR_OVER_R_H_
#define _CALCP_FOR_EKR_OVER_R_H_
...
namespace pfft{
  ...
  class CalcpForEkrOverR{

    typedef vector3D<double> RealVector;
  ...
  public:
    ...
    void operator() (Panel, vector3D<double>, double&, 
  ...
  private:
    ...
    void compIntegralOnTriangle (const vector3D<double>&, double&, 
				 RealVector&);
    void compOuterIntegral(const vector3D<double>&, double, double,
			   double&, double&, 
			   double&); 
    double compSingleLayerInnerIntegral(const double, 
					const double,
					const double, 
					const double);
    double compDIDXInnerIntegral(const double, 
				 const double, 
				 const double,
				 const double, const double, const double, 
				 const double, const double);
    double compDIDYOnTriangle(const vector3D<double>&);
    double compDIDZInnerIntegral(const double, 
				 const double, const double, 
				 const double, const double, const double);

    RealVector transferToPanelLocalCoordSystem(const RealVector& grad1);
    void localDoubleLayerCorrection(double&);
    void flipSign(double&, RealVector&);
	      ...
	      vector3D<double>& grad)
  {
    ...
    slp = 0.;
    RealVector gradSumInPanelLocalCoordSys(ZERO_REAL_VECTOR_3D);
    double totalArea = 0.;   
    for(size_t ii=0; ii<panel.shape(); ++ii){
      ...
      double slpLocal;
      RealVector gradInLocalTriangleCoordSys;
	...
	slp += slpLocal * contributionSign;
      ...
      if ( needDoubleLayerPotential_ || needGradient_ ) {
	RealVector gradInPanelLocalCoordSys;
		       ...
		       double& dIdz)
  {
    ...
    for (size_t jj=0; jj < NumGQpoint_; ++jj) {
      ...
      double R = Rb - Ra;
      double ZRb = exp(IK * Rb);
      double ZRa = exp(IK * Ra);
	...
	if ( needSingleLayerPotential_ ) {
	  double slpLocal = compSingleLayerInnerIntegral(ZRa, ZRb, IK, R);
	  slp += slpLocal * w * d / (r*r);
	}
	    
	if ( needDoubleLayerPotential_ || needGradient_ ) {
	  double dIdzLocal = compDIDZInnerIntegral(ZRb, ZRa, Rb, r, d, h);
      ...
      if( needGradient_ ) {
	double tmp = compDIDXInnerIntegral(ZRb, ZRa, IK, Rb, y, R, r, d);
	dIdx += tmp * w;
      }
    }  
  }
			   ...
			   const double d)
  {
    ...
    if( d == 0. ) {
     ...
     double tmp = compSingleLayerInnerIntegral(ZRa, ZRb, IK, R);
      tmp *= (y*y-d*d)/(r*r*r*r);
		       ...
		       const vector3D<double>& evalPnt)
  {
    ...
    double ZRB = exp(IK * RB);
    double ZRA = exp(IK * RA);
    double ZR0 = exp(IK * R0);

    double fThetaA = compSingleLayerInnerIntegral(ZR0, ZRA, IK, RA-R0);
    double fThetaB = compSingleLayerInnerIntegral(ZR0, ZRB, IK, RB-R0);
   ...
   * to maintain high accuracy, I modefy the results here since I already 
   * know them.
   **********************************************************************/
  template<class KT, class Panel>
  void
				...
				double& dfdn)
  {
	...
	dfdn = -PAI;
      ...
      pfft::warningMessage("calcpForEkrOverR.h : localDoubleLayerCorrection",
			   "Wrong double-layer potential. There might be a bug.");

#ifdef DEBUG
      std::cout << dfdn << endl;
      throw domain_error("calcpForEkrOverR.h : localDoubleLayerCorrection");
#endif
   ...
   **********************************************************************/
  template<class KT, class Panel>
  typename CalcpForEkrOverR<KT, Panel>::RealVector
  CalcpForEkrOverR<KT, Panel>
  ::transferToPanelLocalCoordSystem (
				     const RealVector& grad1)
  {
    return RealVector(grad1.x() * triangleInfo_.X.x() + 
